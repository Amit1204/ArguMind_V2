...
| Sources | retries with jittered backoff | `SOURCE_RETRY_ATTEMPTS` (3) | 429, 5xx and 406 retried, `Retry-After` honoured (capped). 406 was added while chasing the arXiv problem above: cached queries returned 200 and cache misses 406, which first looked like origin rate limiting (2026-09-18/19) and turned out to be the TLS fingerprint rule at the CDN edge (2026-09-23) |
...
| Sources | TLS interop cap (arXiv only) | `ARXIV_TLS_MAX_VERSION` (1.2) | arXiv's CDN answers an empty 406 to TLS 1.3 handshakes that offer ALPN from this image's OpenSSL 3.5 (a fingerprint rule that hit many Python clients in Sept 2026; verified with cache-miss probes: TLS 1.3+ALPN → 406, TLS 1.2 → 200, TLS 1.3 without ALPN → 200, older OpenSSL → 200). The arXiv client is capped at TLS 1.2 with verification unchanged; set `1.3` to lift it once arXiv fixes the rule |
