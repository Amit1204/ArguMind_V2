...
| Sources | retries with jittered backoff | `SOURCE_RETRY_ATTEMPTS` (3) | 429, 5xx and 406 retried, `Retry-After` honoured (capped). 406 was added after arXiv's origin answered empty 406s (later 429s) to **cache-miss** queries for two days while cached queries kept returning 200 — origin-side rate limiting of this address after sustained benchmark traffic, masked by the CDN cache (2026-09-18/19) |
